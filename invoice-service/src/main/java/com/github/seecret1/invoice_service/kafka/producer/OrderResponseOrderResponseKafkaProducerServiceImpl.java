...
public class OrderResponseOrderResponseKafkaProducerServiceImpl implements OrderResponseKafkaProducerService {
    ...
    public void sendNoWait(BaseMessage message) {
        logging(message);
        ProducerRecord<String, BaseMessage> record = getRecord(kafkaProperties.getResponseOrdersTopic(), message);
        ...
        kafkaRetryTemplate.execute(context -> {
            try {
                ...
                ProducerRecord<String, BaseMessage> record =
                        getRecord(kafkaProperties.getResponseOrdersTopic(), message);
